...
This page intends to serve as a documentation guide in OPNFV. OPNFV projects are expected to create a variety of document types,
...
store. OPNFV WiKi is also able to integrate these rendered documents directly allowing projects to use the revision controlled documentation
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^
All contributions to the OPNFV project are done in accordance with the OPNFV licensing requirements. Documentation in OPNFV needs to be contributed
under the `Creative Commons 4.0 <https://creativecommons.org/licenses/by/4.0/>`_  and the `SPDX https://spdx.org/>`_ licence.
...
.. code-block:: bash
 ...
 These lines with licensing information will not be rendered in the html and pdf files.
...
look in these directories and trigger events in these directories when generating documents.
...
^^^^^^^^^^^^^^^^^^^^^
Release documentation is a set of documents that are published for each OPNFV release. These documents are created and developed
...
- Folder for manuals on how to use specific features
...
- Brief introduction to configuring OPNFV with its dependencies.

**Release Notes**: :code:`<repo>/docs/release/release-notes`

- Changes in the new release cycle.
...
Will collect the documentation of the test projects allowing the end user to perform testing towards a OPNFV SUT (System Under Test)
