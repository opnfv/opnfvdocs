...
level testing for development, automated deployment, performance characterization and stress
testing.
...
Pods correspond to the cluster of machines defined by the Pharos specification
(in HA mode this consists of 3 controllers and 2 compute nodes). The results pushed
to the database are associated with individual pods, projects and test cases.
Trying to push results generated from a non-referenced pod will will return
an error message by the Test API

The data model is basic, 5 objects are available:
  * Pods
  * Projects
  * Testcases
  * Results
...
running Jenkins jobs can access to the database. Please note that currently token authorization
is implemented but is not yet enabled.


Test Project Reporting
======================

The reporting page for test projects is http://testresults.opnfv.org/reporting/
 ...
 * the "score" as determined by each test project
 * a "trust" indicator

Additional filters (version, installer, test collection time window,... ) will be
available.
...
test cases. This made it very hard to have a global view of available test cases
from the different test projects. A common view was possible through the API
but it was not user friendly. Test cases per project may be listed by calling:
